...
def run_eval_vs_random(
    path: str, episodes: int, device: torch.device, result_path: Optional[str]
) -> None:
    ...
    for ep in range(episodes):
        ...
        while True:
            ...
            a = greedy_action_masked(policy, obs, m, device)
...
def run_ai_vs_ai(
    ...
    def opp_policy(obs_opp: np.ndarray, mask: np.ndarray) -> int:
        return greedy_action_masked(policy, obs_opp, mask, device)
...
def run_human_cli(path: str, device: torch.device) -> None:
    print(
...
def run_human_widget(path: str, device: torch.device) -> None:
...
def main() -> None:
    ...
    if args.mode == "eval_random":
        run_eval_vs_random(args.model, args.episodes, device, result_path)
    elif args.mode == "ai_vs_ai":
        run_ai_vs_ai(args.model, args.episodes, device, result_path)
    elif args.mode == "human_cli":
        run_human_cli(args.model, device)
    elif args.mode == "human":
        run_human_gui(args.model, device)
    else:
        run_human_gui(args.model, device)
